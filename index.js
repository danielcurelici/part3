...
const personsCount = () => app.get('/api/persons', () => Person.find({}))

app.get('/info', (request,response) => {
  const persons = personsCount().length
  console.log(persons)
  response.send(`Phonebook has info for ${persons} persons
    ${new Date}`)
})
  ...
  Person.findById(request.params.id)
    .then(person => {
      if (person)
        ...
        response.status(404).send({ 'error': 'person does not exist' })
    })
